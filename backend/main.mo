...
actor {
  public func formatMarkdown(text : Text) : async Result.Result<Text, Text> {
    try {
  ...
  public func downloadHTML(text : Text) : async Result.Result<Text, Text> {
    try {
      ...
      let content = switch (await formatMarkdown(text)) {
        case (#ok(formattedText)) formattedText;
        case (#err(errorMsg)) return #err(errorMsg);
      };
      #ok(htmlPrefix # content # htmlSuffix)
    } catch (e) {
      #err(Error.message(e))
    }
  };

  public func viewFormattedMarkdown(text : Text) : async Result.Result<Text, Text> {
    try {
